...
function createApp() {
  ...
  app.get("/healthz", (_req, res) => {
    res.status(200).json({ ok: true, service: "stock-investment-evaluator", asOf: new Date().toISOString() });
  });
...
function startServer(
  ...
  return new Promise((resolve) => {
    const server = app.listen(port, host, () => {
      ...
      console.log(`Stock Investment Evaluator running at http://${host}:${activePort}`);
